...
/** Helper to get the active race ID, accounting for Slime replication. */
function effectiveRace(state: GameState): string {
...
export function sigRestTick(state: GameState): void {
  switch (effectiveRace(state)) {
    case 'spider':
      state.sig = Math.min(SIG_MAX.spider ?? 100, sigCur(state) + 2);
      break;
    case 'golem':
      state.sig = Math.min(SIG_MAX.golem ?? 5, sigCur(state) + GOLEM_BUILD_RATE);
      break;
    case 'slime':
      if (state.sigAbsorb) {
...
export function sigStoneAbsorb(state: GameState): number {
  if (effectiveRace(state) !== 'golem') return 0;
...
export function sigSlimeResist(state: GameState, incomingType: DamageType): number {
  if (effectiveRace(state) !== 'slime' || !state.sigAbsorb) return 0;
